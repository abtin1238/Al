...
## نقشه، جستجو و مسیریابیِ آنلاینِ واقعی (به‌روزرسانیِ جدید)
- نقشه اکنون **آنلاین و واقعی** است (کاشی‌های OpenStreetMap/CARTO از طریق `flutter_map`)، نه یک نقشه‌ی برداریِ فیک.
- جستجو (`GeocodingService`) و مسیریابی (`RoutingService`) به‌صورت **زنده و داینامیک** از سرویس‌های عمومیِ Nominatim و OSRM گرفته می‌شوند — هیچ دیتابیس محلیِ فیکی در مسیرِ جستجو/مسیریابی نیست. برای این بخش پکیج `http` اضافه شده؛ حتماً `flutter pub get` را دوباره اجرا کن.
- انتخابِ مقصد روی نقشه فقط با **لمسِ طولانی** ممکن است (`OnlineMapView.onLongPressLatLng`).
- نوتیفیکیشن‌های بالای اپ (`AppNotificationOverlay`) با همان استایلِ شیشه‌ایِ نوار پایین، جایگزینِ SnackBarهای قبلی شده‌اند.
- **دانلود نقشه‌ی آفلاین** از تنظیمات → «تنظیمات نقشه و لایه‌ها» در دسترس است (`OfflineMapDownloadService` بر پایه‌ی `flutter_map_tile_caching`)؛ فعلاً ایران، ترکیه، امارات و عراق در فهرست هستند و افزودنِ کشورهای دیگر فقط افزودنِ یک `DownloadableRegion` دیگر در `offline_map_download_service.dart` است.
- ⚠️ چون این محیط به اینترنت/کامپایلرِ Flutter دسترسی نداشت، کدها دستی و با دقت نوشته شده‌اند اما **کامپایل نشده‌اند**؛ بعد از `flutter pub get` حتماً یک‌بار `flutter analyze` بگیر — بخشِ پرریسک‌تر از نظر تطابقِ نسخه‌ی پکیج، APIهای `flutter_map_tile_caching` در `offline_map_download_service.dart` است.
